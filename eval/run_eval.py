...
def evaluate_run_with_qrels(run, qrels, exclude=False):
    if exclude:
        output = subprocess.check_output(
            f'python eval/ms_marco_doc_eval.py {run} eval/{qrels} exclude/', shell=True).decode('utf-8')
    else:
        output = subprocess.check_output(
            f'python eval/ms_marco_doc_eval.py {run} eval/{qrels}', shell=True).decode('utf-8')
